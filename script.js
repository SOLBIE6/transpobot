...
async function loadDashTrajets() {
    try {
        ...
        el.innerHTML = `<table class="data-table"><thead><tr>
            <th>Ligne</th><th>Chauffeur</th><th>Véhicule</th><th>Départ</th><th>Statut</th>
        </tr></thead><tbody>
        ...
        console.error('Erreur loadDashTrajets:', e);
    }
}
...
async function loadV() {
    try {
        ...
        <table class="data-table" id="vtbl">
            <thead><tr>
                <th>Immatriculation</th><th>Type</th><th>Capacité</th>
                <th>Statut</th><th>Kilométrage</th><th>Acquisition</th>
            </tr></thead>
            <tbody>${data.map(v => `
                <tr>
        ...
        document.getElementById('vwrap').innerHTML =
            '<div class="empty-state"><i class="fa-solid fa-triangle-exclamation ei"></i>Erreur de chargement</div>';
    }
}
...
async function loadC() {
    try {
        ...
        <table class="data-table" id="ctbl">
            <thead><tr>
                <th>Nom complet</th><th>N° Permis</th><th>Catégorie</th>
                <th>Téléphone</th><th>Véhicule assigné</th><th>Embauche</th>
            </tr></thead>
            <tbody>${data.map(c => `
                <tr>
                    ...
                    <td>${c.telephone || '—'}</td>
                    <td>${c.immatriculation
                        ? `<span style="color:var(--accent)">${c.immatriculation}</span>`
                        : '<span style="color:var(--text3)">Non assigné</span>'}</td>
...
async function loadT() {
    try {
        ...
        <table class="data-table" id="ttbl">
            <thead><tr>
                <th>Ligne</th><th>Chauffeur</th><th>Véhicule</th>
                <th>Départ</th><th>Arrivée</th><th>Statut</th><th>Recette</th>
            </tr></thead>
            <tbody>${data.map(t => `
                <tr>
                    <td style="color:var(--text);font-weight:500">${t.ligne}</td>
...
async function loadI() {
    try {
        const data = await api('/api/incidents'); // ✅ route désormais disponible côté backend
        if (!data || !data.length) {
            document.getElementById('iwrap').innerHTML =
                '<div class="empty-state"><i class="fa-solid fa-check-circle ei" style="color:var(--green)"></i>Aucun incident signalé</div>';
            return;
        }
        document.getElementById('iwrap').innerHTML = `
        <table class="data-table" id="itbl">
            <thead><tr>
                <th>Type</th><th>Description</th><th>Gravité</th>
                <th>Date</th><th>Statut</th><th>Chauffeur</th><th>Ligne</th>
            </tr></thead>
            <tbody>${data.map(i => `
                <tr>
                    ...
                    <td>${i.resolu
                        ? '<span class="badge badge-actif">Résolu</span>'
                        : '<span class="badge badge-hors_service">Ouvert</span>'}</td>
...
function addMsg(role, html, sql = null) {
    ...
    d.innerHTML = `
        <div class="chat-av">${role === 'bot' ? '🤖' : '👤'}</div>
        <div>
            <div class="bubble">${html}</div>
            ${sql ? `<div class="sql-chip">${sql}</div>` : ''}
        </div>`;
    box.appendChild(d);
...
function loadExampleQuestions() {
    ...
    container.innerHTML = questions.map(q => `
        <button class="sug"
            style="text-align:left;border-radius:8px;padding:6px 10px;width:100%"
            onclick="ask('${q.replace(/'/g, "\\'")}')">
            ${q}
        </button>
    `).join('');
}
